#!/bin/bash
#
# Copyright 2017 Frederic Lavigne.
#
...
function install() {
  ...
  ./wsk action create $PACKAGE_NAME/noOp\
    actions/options.js
}
...
function uninstall() {
  ...
  ./wsk action delete $PACKAGE_NAME/noOp
...
function update() {
  ...
  ./wsk action update $PACKAGE_NAME/noOp              actions/options.js
}
...
function installApi() {
  ./wsk api-experimental create /emoting/1 /questions       PUT     emoting/questionCreate
  ./wsk api-experimental create /emoting/1 /questions       GET     emoting/questionRead
  ./wsk api-experimental create /emoting/1 /questions       OPTIONS emoting/noOp

  ./wsk api-experimental create /emoting/1 /stats           GET     emoting/questionStats
  ./wsk api-experimental create /emoting/1 /stats           OPTIONS emoting/noOp

  ./wsk api-experimental create /emoting/1 /ratings         PUT     emoting/ratingCreate
  ./wsk api-experimental create /emoting/1 /ratings         OPTIONS emoting/noOp
}

function uninstallApi() {
  ./wsk api-experimental delete /emoting/1
}
